  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className="flex flex-col gap-4">
        ...
        <img onClick={() => fileRef.current.click()} src={formData.avatar || currentUser.avatar} alt='profile' className="rounded-full h-24 w-24 object-cover curser-pointer self-center mt-2" />
        <p className="text-sm self-center">
          {fileuploadError ? (
            <span className="text-red-500">Error Image upload</span>)
            : filePerc > 0 && filePerc < 100 ? (
              <span className='text-slate-800'>{`Uploading ${filePerc}%`}
              </span>
            ) : filePerc === 100 ? (
